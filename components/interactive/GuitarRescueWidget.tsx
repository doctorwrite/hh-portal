// components/interactive/GuitarRescueWidget.tsx
'use client'

import { useState, useRef, useEffect, useCallback } from 'react'

interface GuitarRescueParams {
  mode: 'before' | 'after' | 'compare'
  intensity: number
}

const GuitarRescueWidget: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [params, setParams] = useState<GuitarRescueParams>({
    mode: 'compare',
    intensity: 50,
  })
  ...
  const renderWidget = useCallback(() => {
    ...
    ctx.fillText(`🎸 Спасение гитары — ${modeLabels[params.mode]}`, margin, 2)

    // === Волна ===
    const centerY = graphY + graphH / 2
    const steps = 300

    // До обработки (грязный, шумный, без атаки)
    const beforeAmp = 20
    const afterAmp = 28

    // Рисуем "До" (красная, грязная)
    ctx.beginPath()
    for (let i = 0; i <= steps; i++) {
      const t = (i / steps) * 4 * Math.PI
      const x = graphX + (i / steps) * graphW
      let val = Math.sin(t * 0.8 + 0.2) * 0.25 + Math.sin(t * 1.6 + 0.5) * 0.08
      val += (Math.random() - 0.5) * 0.2 // шум
      // Отсутствие атаки (размытые ноты)
      val += 0.05 * Math.sin(t * 0.3)
      // Грязь (дополнительные искажения)
      if (t > 0.4 && t < 0.6) val += 0.1
    ...
    // Рисуем "После" (зелёная, чистая, с атакой)
    ctx.beginPath()
    for (let i = 0; i <= steps; i++) {
      const t = (i / steps) * 4 * Math.PI
      const x = graphX + (i / steps) * graphW
      let val = Math.sin(t * 0.8 + 0.2) * 0.3 + Math.sin(t * 1.6 + 0.5) * 0.1
      // Чистая атака (чёткие пики)
      if (t > 0.15 && t < 0.2) val += 0.15
      if (t > 0.55 && t < 0.6) val += 0.12
      // Чистый звук, без шума и грязи
    ...
    const infoText = params.mode === 'before'
      ? '❌ Грязный звук, шум, отсутствие атаки'
      : params.mode === 'after'
      ? '✅ Чисто, плотно, профессионально'
  ...
  const updateParam = useCallback((key: keyof GuitarRescueParams, value: any) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#888', letterSpacing: '1px' }}>
          <span style={{ color: '#f5c542' }}>HH</span>Records · Guitar Rescue
        </span>
      ...
      <div style={{ textAlign: 'center', fontSize: '0.4rem', color: '#555', padding: '4px 0' }}>
        🎸 Сравните гитарный звук до и после профессиональной обработки
      </div>
...
export default GuitarRescueWidget