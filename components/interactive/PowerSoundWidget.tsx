// components/interactive/PowerSoundWidget.tsx
'use client'

import { useState, useRef, useEffect, useCallback } from 'react'

interface PowerSoundParams {
  mode: 'before' | 'after' | 'compare'
  intensity: number
}

const PowerSoundWidget: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [params, setParams] = useState<PowerSoundParams>({
    mode: 'compare',
    intensity: 50,
  })
  ...
  const renderWidget = useCallback(() => {
    ...
    ctx.fillText(`🔊 Создание мощного звука — ${modeLabels[params.mode]}`, margin, 2)

    // === Волна ===
    const centerY = graphY + graphH / 2
    const steps = 300

    // До обработки (плоская, узкая, тихая)
    const beforeAmp = 15
    const afterAmp = 28

    // Рисуем "До" (красная, плоская)
    ctx.beginPath()
    for (let i = 0; i <= steps; i++) {
      const t = (i / steps) * 4 * Math.PI
      const x = graphX + (i / steps) * graphW
      let val = Math.sin(t * 0.8 + 0.2) * 0.2 + Math.sin(t * 1.6 + 0.5) * 0.06
      val += 0.02 * Math.sin(t * 0.3)
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      // Мощные пики
      if (t > 0.2 && t < 0.25) val += 0.1
      if (t > 0.55 && t < 0.6) val += 0.08
    ...
    const infoText = params.mode === 'before'
      ? '❌ Плоский, узкий, тихий звук'
      : params.mode === 'after'
      ? '✅ Плотный, мощный, громкий звук'
  ...
  const updateParam = useCallback((key: keyof PowerSoundParams, value: any) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#888', letterSpacing: '1px' }}>
          <span style={{ color: '#f5c542' }}>HH</span>Records · Power Sound
        </span>
      ...
      <div style={{ textAlign: 'center', fontSize: '0.4rem', color: '#555', padding: '4px 0' }}>
        🔊 Сравните звук до и после создания "полного" и мощного звука
      </div>
...
export default PowerSoundWidget